...
export class Ratelimiter {
   ...
   */
  static check(request: Request, route: string): RateLimitResponse | undefined {
    ...
    const routeConfig: RateLimitConfig | undefined = this.routeConfigs.get(route);
    if (!routeConfig) return undefined;

    // Get the user's IP address to use as the key
    const ip: string = request.headers.get('CF-Connecting-IP') || request.headers.get('X-Forwarded-For') || '127.0.0.1';
    const key = `${route}:${ip}`;
    ...
    // Return the response
    const remaining: number = Math.max(0, routeConfig.maxRequests - limitInfo.count);
   ...
   */
  static applyHeaders(response: Response, rateLimitResponse: RateLimitResponse) {
    response.headers.set('X-RateLimit-Remaining', `${rateLimitResponse.remainingRequests}`);
    response.headers.set('X-RateLimit-Reset', `${rateLimitResponse.resetTime}`);
    return response;
  }
}
